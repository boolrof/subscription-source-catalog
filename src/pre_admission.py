...
SCHEMES = {"vless", "vmess", "trojan", "ss", "ssr", "hysteria2", "hy2", "tuic"}
...
def resolve_public(host: str) -> list[str]:
    ...
    return [ip for ip in ips if _global_ip(ip)]
...
def _decode_text(body: bytes) -> str:
    ...
    if len(compact) >= 16 and len(compact) <= MAX_DECODED * 2 and re.fullmatch(r"[A-Za-z0-9+/=_-]+", compact):
        try:
...
def parse_nodes(body: bytes, *, max_nodes: int = 10000) -> tuple[list[NodeFact], dict]:
    ...
    if not raw_items and ("proxies:" in text or SERVER_RE.search(text)):
        detected = "mihomo"
        servers = SERVER_RE.findall(text)
...
class GeoResolver:
    ...
    def country(self, ip: str | None) -> str | None:
        ...
        with self._lock:
            if ip in self.cache:
                return self.cache[ip] or None
            if self._new >= self.max_new:
                return None
            self._new += 1
        try:
            req = urllib.request.Request(
                "https://api.country.is/" + urllib.parse.quote(ip, safe=":"),
                headers={"User-Agent": "subscription-source-catalog/compute-v2"},
            )
...
def source_score(stats: dict, *, content_changed: bool, duplicate: bool = False) -> int:
    score = 0
    if stats.get("fetch_status") == "success":
        score += 25
    valid = int(stats.get("valid_nodes") or 0)
    raw = int(stats.get("raw_items") or 0)
    invalid = int(stats.get("invalid_items") or 0)
    if valid:
        score += 20
    if valid >= 20:
        score += 15
    if raw and invalid / max(raw, 1) <= 0.10:
        score += 10
    score += min(10, max(0, len(stats.get("protocol_counts") or {}) * 3))
    if content_changed:
        score += 10
    if stats.get("endpoint_country_counts"):
        score += 10
    if duplicate:
        score -= 20
    ...
    try:
        ...
        stats = {
            **parse,
            ...
            "endpoint_country_counts": dict(sorted(country_counts.items())),
        }
...
def inspect_many(items: list[dict], *, max_sources: int, max_bytes: int, timeout: float, workers: int, geo_cache: dict[str, str], geo_max_new: int) -> list[dict]:
    ...
    def one(item):
        return inspect_source(item, max_bytes=max_bytes, timeout=timeout, geo=geo)
