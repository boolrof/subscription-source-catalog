...
class ComputeMergeTests(unittest.TestCase):
    def run_merge(self, root, sources, results, top=30, max_per_source=5):
        ...
        argv = [
            ...
            "--handoff", str(handoff), "--prechecked", str(prechecked),
            "--nodes-deduplicated", str(nodes), "--countries-dir", str(countries),
            "--top-per-country", str(top), "--max-per-source", str(max_per_source),
        ]
        with mock.patch("sys.argv", argv):
            self.assertEqual(merge_compute.main(), 0)
        return handoff, nodes, countries
    ...
    def test_country_handoff_contains_only_safe_facts(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            handoff, nodes, countries = self.run_merge(
                root,
                [{"url": url, "repository": "example/repo", "status": "active", "format_hint": "uri"}],
                [{
                    "source_id": "source123",
                    "url": url,
                    "precheck": {
            ...
            rendered = handoff.read_text(encoding="utf-8") + nodes.read_text(encoding="utf-8")
    ...
    def test_global_dedup_collapses_same_node_across_sources(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            handoff, nodes_path, _ = self.run_merge(root, sources, results)
    ...
    def test_mirror_sources_do_not_inflate_independent_support(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            _, nodes_path, _ = self.run_merge(root, sources, results)
